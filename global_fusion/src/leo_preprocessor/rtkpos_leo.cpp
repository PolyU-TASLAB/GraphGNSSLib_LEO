                  ...
                  const nav_t *nav)
{
    ...
    for(int is=0; is<ns; is++) 
    {
        ...
        // if(is>=nu)
        {
            ...
            gnss_raw.lambda = nav->lam[obs[ir[is]].sat][0];
    ...
    for(int is=0; is<ns; is++) 
    {
        ...
        // if(is>=nu)
        {
            ...
            gnss_raw.raw_pseudorange = obs[ir[is]].P[0];
            gnss_raw.carrier_phase = obs[ir[is]].L[0];
            gnss_raw.lambda = nav->lam[obs[ir[is]].sat-1][0];
            ...
            gnss_raw_mf.prn_satellites_index = float(obs[ir[is]].sat);
            gnss_raw_mf.azimuth = azel[0] * R2D;
            gnss_raw_mf.elevation = azel[1] * R2D;
            ...
            for (int j=0; j<NFREQ+NEXOBS;j++) {
                gnss_raw_mf.lambda.push_back(nav->lam[obs[ir[is]].sat][j]);
                gnss_raw_mf.pseudorange.push_back(1.0);
                gnss_raw_mf.raw_pseudorange.push_back(obs[ir[is]].P[j]);
                gnss_raw_mf.raw_carrier_phase.push_back(obs[ir[is]].L[j]);
                gnss_raw_mf.doppler.push_back(obs[ir[is]].D[j]);
                gnss_raw_mf.snr.push_back(obs[ir[is]].SNR[j] * 0.25);
            }
            gnss_data.GNSS_Raws_mf.push_back(gnss_raw_mf);

            int sys=satsys(obs[ir[is]].sat,NULL);
        }
